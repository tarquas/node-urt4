...
char demoPfx[MAX_OSPATH] = "unknown";

void urt4_cleanName(char *name);
void urt4_stopDemo(struct client_s *client);

char demoFiles[MAX_CLIENTS][MAX_OSPATH] = {0};

Urt4_Hook(void, SV_StopRecordOne, (client_t *client)) {
  int id = (int)(client - svs.clients);
  demoFiles[id][0] = 0;
  urt4_SV_StopRecordOne(client);
}

Urt4_Hook(void, SV_StartRecordOne, (client_t *client, char *filename)) {
  if (filename) { urt4_SV_StartRecordOne(client, filename); return; }
  ...
  id = (int)(client - svs.clients);

  Com_RealTime(&time);

  s = Info_ValueForKey(client->userinfo, "authl");

  if (*client->auth) {
    Q_strncpyz(who, client->auth, sizeof(who));
  } else if (s && *s) {
    Q_strncpyz(who, s, sizeof(who));
  } else {
    Q_strncpyz(playername, client->name, sizeof(playername));
    urt4_cleanName(playername);

    s = (char *) NET_AdrToString(client->netchan.remoteAddress);
    urt4_cleanName(s);

    Com_sprintf(who, sizeof(who), "%s-%s", s, playername);
  }

  Com_sprintf(
    path, MAX_OSPATH, "%.4d-%.2d-%.2d_%.2d-%.2d-%.2d_%s-%s",
    time.tm_year+1900, time.tm_mon + 1, time.tm_mday,
    time.tm_hour, time.tm_min, time.tm_sec,
    sv_mapname->string, who
  );

  strcpy(demoFiles[id], path);

  s = sv_demofolder->string;
  sv_demofolder->string = demoPfx;
  urt4_SV_StartRecordOne(client, path);
  sv_demofolder->string = s;
}

// ClientThink

static cvar_t *cvarGhostMask = 0;
...
void SV_GhostThink(client_t *cl) {
  ...
  if (!cvarGhostMask) cvarGhostMask = Cvar_Get("nodeurt_ghostmask", "-1", 0);

  id = (int)(cl - svs.clients);
  if (sv_gametype->integer != 9 || !(cvarGhostMask->integer & (1 << id))) return;
...
Urt4_Hook(void, SV_Auth_DropClient, (client_t *drop, const char *reason, const char *message)) {
  ...
  //UPSTREAM: if (drop->demo_recording) urt4_stopDemo(drop); // fix demo
...
Urt4_Hook(void, SV_DropClient, (client_t *drop, const char *reason)) {
  ...
  if (drop->demo_recording) urt4_stopDemo(drop); // fix demo
...
Urt4_Hook(void, SV_SpawnServer, (char *server, qboolean killBots)) {
  ...
  Com_sprintf(
    demoPfx, MAX_OSPATH, "%s/%.4d-%.2d-%.2d/%.4d-%.2d-%.2d_%.2d-%.2d-%.2d_%s",
...
int Api_exec_sv(long long rpcId, const char *cmd) {
  ...
  if (!memcmp(cmd, "getdemofile ", 12)) {
    ...
    char msg2[1024];
    snprintf(msg2, 1024, "rpc %lld %s", rpcId, demoFiles[slot]);
    Api_send(msg2);
    return 1;
  }

  if (!memcmp(cmd, "getdemopfx", 9)) {
    char msg2[1024];
    snprintf(msg2, 1024, "rpc %lld %s", rpcId, demoPfx);
    Api_send(msg2);
    return 1;
  }
